...
func main() {
	...
	// response from `GetGamegenres`: QueryResultBaseItemDto
...
> BaseItemDto GetGamegenresByName(ctx, name).UserId(userId).Execute()
...
func main() {
	...
	// response from `GetGamegenresByName`: BaseItemDto
