...
> []PackageInfo GetPackages(ctx).PackageType(packageType).TargetSystems(targetSystems).IsPremium(isPremium).IsAdult(isAdult).Execute()
...
> PackageInfo GetPackagesByName(ctx, name).AssemblyGuid(assemblyGuid).Execute()
...
func main() {
	...
	// response from `GetPackagesByName`: PackageInfo
...
> []PackageVersionInfo GetPackagesUpdates(ctx).PackageType(packageType).Execute()
...
func main() {
	...
	// response from `GetPackagesUpdates`: []PackageVersionInfo
...
func main() {
	...
	updateClass := openapiclient.PackageVersionClass("Release") // PackageVersionClass | Optional update class (Dev, Beta, Release). Defaults to Release. (optional)
 ...
 **updateClass** | [**PackageVersionClass**](PackageVersionClass.md) | Optional update class (Dev, Beta, Release). Defaults to Release. | 
