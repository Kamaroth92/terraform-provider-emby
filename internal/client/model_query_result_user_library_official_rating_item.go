/*
Emby Server REST API (BETA)

Explore the Emby Server API

API version: 4.9.4.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package embyclient

import (
	"encoding/json"
)

// checks if the QueryResultUserLibraryOfficialRatingItem type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &QueryResultUserLibraryOfficialRatingItem{}

// QueryResultUserLibraryOfficialRatingItem struct for QueryResultUserLibraryOfficialRatingItem
type QueryResultUserLibraryOfficialRatingItem struct {
	Items []UserLibraryOfficialRatingItem `json:"Items,omitempty"`
	TotalRecordCount *int32 `json:"TotalRecordCount,omitempty"`
}

// NewQueryResultUserLibraryOfficialRatingItem instantiates a new QueryResultUserLibraryOfficialRatingItem object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewQueryResultUserLibraryOfficialRatingItem() *QueryResultUserLibraryOfficialRatingItem {
	this := QueryResultUserLibraryOfficialRatingItem{}
	return &this
}

// NewQueryResultUserLibraryOfficialRatingItemWithDefaults instantiates a new QueryResultUserLibraryOfficialRatingItem object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewQueryResultUserLibraryOfficialRatingItemWithDefaults() *QueryResultUserLibraryOfficialRatingItem {
	this := QueryResultUserLibraryOfficialRatingItem{}
	return &this
}

// GetItems returns the Items field value if set, zero value otherwise.
func (o *QueryResultUserLibraryOfficialRatingItem) GetItems() []UserLibraryOfficialRatingItem {
	if o == nil || IsNil(o.Items) {
		var ret []UserLibraryOfficialRatingItem
		return ret
	}
	return o.Items
}

// GetItemsOk returns a tuple with the Items field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *QueryResultUserLibraryOfficialRatingItem) GetItemsOk() ([]UserLibraryOfficialRatingItem, bool) {
	if o == nil || IsNil(o.Items) {
		return nil, false
	}
	return o.Items, true
}

// HasItems returns a boolean if a field has been set.
func (o *QueryResultUserLibraryOfficialRatingItem) HasItems() bool {
	if o != nil && !IsNil(o.Items) {
		return true
	}

	return false
}

// SetItems gets a reference to the given []UserLibraryOfficialRatingItem and assigns it to the Items field.
func (o *QueryResultUserLibraryOfficialRatingItem) SetItems(v []UserLibraryOfficialRatingItem) {
	o.Items = v
}

// GetTotalRecordCount returns the TotalRecordCount field value if set, zero value otherwise.
func (o *QueryResultUserLibraryOfficialRatingItem) GetTotalRecordCount() int32 {
	if o == nil || IsNil(o.TotalRecordCount) {
		var ret int32
		return ret
	}
	return *o.TotalRecordCount
}

// GetTotalRecordCountOk returns a tuple with the TotalRecordCount field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *QueryResultUserLibraryOfficialRatingItem) GetTotalRecordCountOk() (*int32, bool) {
	if o == nil || IsNil(o.TotalRecordCount) {
		return nil, false
	}
	return o.TotalRecordCount, true
}

// HasTotalRecordCount returns a boolean if a field has been set.
func (o *QueryResultUserLibraryOfficialRatingItem) HasTotalRecordCount() bool {
	if o != nil && !IsNil(o.TotalRecordCount) {
		return true
	}

	return false
}

// SetTotalRecordCount gets a reference to the given int32 and assigns it to the TotalRecordCount field.
func (o *QueryResultUserLibraryOfficialRatingItem) SetTotalRecordCount(v int32) {
	o.TotalRecordCount = &v
}

func (o QueryResultUserLibraryOfficialRatingItem) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o QueryResultUserLibraryOfficialRatingItem) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Items) {
		toSerialize["Items"] = o.Items
	}
	if !IsNil(o.TotalRecordCount) {
		toSerialize["TotalRecordCount"] = o.TotalRecordCount
	}
	return toSerialize, nil
}

type NullableQueryResultUserLibraryOfficialRatingItem struct {
	value *QueryResultUserLibraryOfficialRatingItem
	isSet bool
}

func (v NullableQueryResultUserLibraryOfficialRatingItem) Get() *QueryResultUserLibraryOfficialRatingItem {
	return v.value
}

func (v *NullableQueryResultUserLibraryOfficialRatingItem) Set(val *QueryResultUserLibraryOfficialRatingItem) {
	v.value = val
	v.isSet = true
}

func (v NullableQueryResultUserLibraryOfficialRatingItem) IsSet() bool {
	return v.isSet
}

func (v *NullableQueryResultUserLibraryOfficialRatingItem) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableQueryResultUserLibraryOfficialRatingItem(val *QueryResultUserLibraryOfficialRatingItem) *NullableQueryResultUserLibraryOfficialRatingItem {
	return &NullableQueryResultUserLibraryOfficialRatingItem{value: val, isSet: true}
}

func (v NullableQueryResultUserLibraryOfficialRatingItem) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableQueryResultUserLibraryOfficialRatingItem) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


