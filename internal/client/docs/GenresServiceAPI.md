...
func main() {
	...
	// response from `GetGenres`: QueryResultBaseItemDto
...
> BaseItemDto GetGenresByName(ctx, name).UserId(userId).Execute()
...
func main() {
	...
	// response from `GetGenresByName`: BaseItemDto
