...
func main() {
	...
	// response from `GetStudios`: QueryResultBaseItemDto
...
> BaseItemDto GetStudiosByName(ctx, name).UserId(userId).Execute()
...
func main() {
	...
	// response from `GetStudiosByName`: BaseItemDto
