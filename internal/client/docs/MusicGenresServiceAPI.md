...
func main() {
	...
	// response from `GetMusicgenres`: QueryResultBaseItemDto
...
> BaseItemDto GetMusicgenresByName(ctx, name).UserId(userId).Execute()
...
func main() {
	...
	// response from `GetMusicgenresByName`: BaseItemDto
